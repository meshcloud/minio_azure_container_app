locals {
  ...
  dns_challenge_annotations = var.lets_encrypt_challenge == "dns" ? {
    ...
    "bunkerweb.io/LETS_ENCRYPT_DNS_CREDENTIAL_ITEM" = var.lets_encrypt_dns_credential_item
  } : {}
...
resource "kubernetes_config_map" "seaweedfs_modsec" {
  metadata {
    ...
    annotations = {
      ...
      "bunkerweb.io/CONFIG_SITE" = local.seaweedfs_fqdn
    }
  }
...
resource "kubernetes_config_map" "keycloak_modsec" {
  metadata {
    ...
    annotations = {
      "bunkerweb.io/CONFIG_TYPE" = "modsec-crs"
      "bunkerweb.io/CONFIG_SITE" = local.keycloak_fqdn
    }
  }
